...
- This branch is `personal-macos`, the user's Apple Silicon macOS installation. The separate `personal` branch is for Fedora.
...
- Assume Apple Silicon and the PyTorch MPS backend unless inspection shows otherwise; do not install CUDA-only packages here.
- Launch with `conda activate comfyenv` followed by `./start.sh`. Do not install ComfyUI packages into the unrelated system or python.org Python.
- Preserve the local MPS frame-interpolation fixes unless upstream adds equivalent support.
- Ask before updating ComfyUI, Python packages, custom nodes, or large models because those actions can break a working setup or consume significant bandwidth and disk space.

## Git and Updates

- `origin` is the user's fork and `upstream` is the official ComfyUI repository. Treat `upstream` as fetch-only and never push to it.
- Stay on `personal-macos` unless the user explicitly requests another branch.
- When the user asks to update this checkout, integrate `upstream/master` into `personal-macos` while preserving the macOS-specific files and fixes.
- Push only to `origin` and only when the user explicitly requests it. Do not open upstream pull requests for personal changes.

## Primary Goal

Help the user run ComfyUI and generate AI content successfully. Typical work includes:

- Diagnosing startup errors, crashes, dependency problems, MPS or memory issues, model-loading failures, and custom-node conflicts.
